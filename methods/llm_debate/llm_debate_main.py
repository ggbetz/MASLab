...
class LLM_Debate_Main(MAS):
    ...
    async def inference(self, sample):
            ...
            for i, agent_context in enumerate(agent_contexts):
                if round != 0:
                    ...
                    message = self.construct_message(
                        agent_contexts_other, query, 2 * round - 1
                    )
        ...
        final_answer = await self.aggregate(query, answers)
    ...
    async def aggregate(self, query, answers):
        ...
        response = await self.call_llm(prompt=aggregate_instruction)
        return response
