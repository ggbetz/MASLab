...
class SelfConsistency(MAS):
    ...
    async def inference(self, sample):
        ...
        agent_results = [
            await self.call_llm(prompt=query) for _ in range(self.parallel_num)
        ]

        final_decision_instruction = self.get_final_decision_instruction(
            query, agent_results
        )
        response = await self.call_llm(prompt=final_decision_instruction)
