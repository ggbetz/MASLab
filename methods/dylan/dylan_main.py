...
class DyLAN_Main(MAS):
    ...
    def __init__(self, general_config, method_config_name=None):
        ...
        self.activation = activation_map.get(
            self.method_config.get("activation", "listwise"), 0
        )
        ...
        self.cmp_res = (
            lambda x, y: self._sentence_bleu(x, [y], lowercase=True) >= 0.9 * 100
        )
    ...
    async def inference(self, sample) -> dict[str, str]:
        ...
        for idx, node_idx in enumerate(loop_indices):
            await self._activate_node(node_idx, query)
            activated_indices.append(node_idx)
            if idx >= math.floor(2 / 3 * self.num_agents):
                reached, reply = self._check_consensus(
                    activated_indices, list(range(self.num_agents))
                )
                if reached:
                    return {"response": "" if reply is None else str(reply)}
        ...
        for rid in range(2, self.num_rounds):
            if self.num_agents > 3:
                ...
                if self.activation == 0:  # listwise
                    tops = await self._listwise_ranker(shuffled_replies, query)
                    idx_mask = list(
                        map(lambda x: idxs[indices[x]] % self.num_agents, tops)
                    )

            loop_indices = list(
                range(self.num_agents * rid, self.num_agents * (rid + 1))
            )
            random.shuffle(loop_indices)
            idxs = []
            for idx, node_idx in enumerate(loop_indices):
        ...
        response, _ = self._most_frequent(
            [self.nodes[idx]["answer"] for idx in idxs], self.cmp_res
        )
    ...
    def get_activation_map(self) -> dict[str, int]:
    ...
    async def _activate_node(self, idx, question):
        """Activate a node and process its response using async LLM calls."""
        node = self.nodes[idx]
        ...
        agent_id = f"dylan_main_{node['role']}"
        ...
        for eid, weight in zip(original_idxs, weights):
            self.edges[eid]["weight"] = (
                weight / 5 if 0 < weight <= 5 else (1 if weight > 5 else 0)
            )
    ...
    async def _listwise_ranker(self, responses, question):
        ...
        completion = await self.call_llm_for_agent_async(
            agent_id="dylan_main_ranker", messages=[message]
        )
        return self._parse_ranks(completion, max_num=len(responses))

    def _most_frequent(
        self, clist: List[str], cmp_func: Callable[[str, str], bool]
    ) -> Tuple[str, int]:
        ...
        for candidate in clist:
            current_count = sum(1 for item in clist if cmp_func(candidate, item))
