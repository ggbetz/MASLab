...
class DyLAN_MATH(MAS):
    ...
    def __init__(self, general_config, method_config_name=None):
        ...
        method_config_name = (
            "config_math" if method_config_name is None else method_config_name
        )
    ...
    async def inference(self, sample) -> dict[str, str]:
        ...
        for i, agent_context in enumerate(agent_contexts):
            ...
            if i >= math.floor(
                2 / 3 * len(agent_contexts)
            ) and self._check_reach_consensus(agent_contexts[: i + 1]):
                consensus = True
                break
        ...
        wrapped_answer = (
            r"\boxed{" + answer + "}"
        )  # For fair comparison between evaluation protocols
    ...
    def _construct_message(
        self, agents: List[List[Dict[str, str]]], question: str
    ) -> str:
    ...
    def _construct_ranking_message(
        self, agents: List[List[Dict[str, str]]], question: str
    ) -> str:
    ...
    def _check_reach_consensus(
        self, agent_contexts: List[List[Dict[str, str]]]
    ) -> bool:
    ...
    def _extract_final_answer(self, agent_contexts: List[List[Dict[str, str]]]) -> str:
