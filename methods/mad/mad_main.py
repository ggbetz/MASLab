...
class MAD_Main(MAS):
    ...
    def init_prompt(self, debate_topic):
        ...
        self.player_meta_prompt = PLAYER_META_PROMPT.replace(
            "##debate_topic##", debate_topic
        )
        self.moderator_prompt = MODERATOR_META_PROMPT.replace(
            "##debate_topic##", debate_topic
        )
        self.affirmative_prompt = AFFIRMATIVE_PROMPT.replace(
            "##debate_topic##", debate_topic
        )
        self.judge_prompt_last2 = JUDGE_PROMPT_LAST2.replace(
            "##debate_topic##", debate_topic
        )
    ...
    async def init_agents(self):
        ...
        try:
            ...
            raise ValueError(
                f"Moderator did not return valid JSON: {self.mod_ans}"
            ) from e
    ...
    async def inference(self, sample):
        ...
        for round in range(self.max_round - 1):
            if isinstance(self.mod_ans, dict) and self.mod_ans.get(
                "debate_answer"
            ):  # if the debate is done, stop the loop
                break
            else:
        ...
        if isinstance(self.mod_ans, dict) and self.mod_ans.get("debate_answer"):
            ...
            self.config.update(dict(self.mod_ans))
            ...
            judge_player.add_event(
                JUDGE_PROMPT_LAST1.replace("##aff_ans##", aff_ans).replace(
                    "##neg_ans##", neg_ans
                )
            )
            ...
            try:
                ...
                raise ValueError(f"Judge did not return valid JSON: {ans}") from e
